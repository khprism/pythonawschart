...
class AWSChart(object):
    ...
    def _get_datapoints(self):
        if not self._datapoints:
            metrics = self.cloudwatch_connection.list_metrics()
            my_metrics = []
            for metric in metrics:
                if 'InstanceId' in metric.dimensions:
                    if self.instance in metric.dimensions['InstanceId']:
                        my_metrics.append(metric)
            index = 0
            for n,i in enumerate(my_metrics):
                if str(i) == self.args['metric']:
                    index = n
                    break;
            metric = my_metrics[index]
            self._datapoints = metric.query(
    ...
    def _get_chart(self):
        if not self._chart:
            ...
            data = [datapoint['Maximum'] for datapoint in self._datapoints]
